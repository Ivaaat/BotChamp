...
def parent_word(month):
    #date_match = datetime.strptime(date.split()[0], '%Y-%m-%d').strftime('%d %B %A')
...
class Calendar(Championat):
    def __init__(self, country=""):
        ...
        num_team = self.tree.xpath('//ul [@class="entity-header__facts js-entity-header-facts"]/li[3]/text()')
        self.num_team = int(int(num_team[1].strip())/2)

    def get_date(self):
        return [" ".join(date.replace('.','-').split()) for date in self.tree.xpath(date_xpath_match)]
    
    def get_tour(self):
        return self.tree.xpath(num_xpath_tour)
    
    def get_matches(self):
        return [match.split(",")[0] for match in self.tree.xpath('//td [@class="stat-results__link"]//@title')]
    
    def get_result(self):
        return [result.strip() for result in self.tree.xpath(parse_xpath_result)]
...
class Table(Championat):
    def __init__(self,country):
        ...
        response = sess.get(url_table)  
        self.response_text = response.text
        self.tree = html.fromstring(self.response_text)
        self.dict_table = {}
    
    def get_name(self):
        names_teams = self.tree.xpath('//span[@class="table-item__name"]/text()')
        return names_teams[:int(len(names_teams)/3)]

    def get_points(self):
        num_points = [ball.strip() for ball in self.tree.xpath('//td[8]/strong/text()')]
    ...
    def get_draw(self):
            num_draw = [draw.strip() for draw in self.tree.xpath('//td[5]//text()')]
            return num_draw[:int(len(num_draw)/3)]
            
    def get_loss(self):
        num_loss = [lose.strip() for lose in self.tree.xpath('//td[6]//text()')]
        return num_loss[:int(len(num_loss)/3)]

    def get_balls(self):
        num_balls = [ball.strip() for ball in self.tree.xpath('//td[7]//text()')]
...
def add_db(name, name_champ):
    ...
    for i, j in enumerate(range(0, len(calendar_all), calendar.num_team), 1):
        ...
        dict_calendar[f'Тур {i}'] = {
            ...
            'start':datetime.strptime(sort_date(matches[0][1][0]), '%d-%m-%Y %H:%M'),
            'end': datetime.strptime(sort_date(matches[calendar.num_team - 1][1][0]), '%d-%m-%Y %H:%M'),
            'Закончен':ends,
                        }
    country.update_one({"Чемпионат": '2022/2023'}, {'$set':{'Календарь':dict_calendar}})
    table = Table(name)
    ...
    for i, name_team in enumerate(table.get_name()):
        games = [" | ".join(namea[1]) for namea in calendar_all if name_team in namea[1][1]]
    ...
    country.update_one({"Чемпионат": '2022/2023'}, {'$set':dict_table})
    ...
    try:
        for team, stat in table.items():
            if team not in ['_id','Чемпионат','Календарь','Лого']:
                res_six_match =[teams for teams in stat['Календарь'] if not teams.endswith("– : –")]
        ...
        clear_table.sort(key=lambda b: (int(b[1][1]), 
                                        int(b[1][2].split("-")[0]) -  int(b[1][2].split("-")[1])))
        clear_table.reverse()
            ...
            for name_match in tour['Матчи']:
                ...
                for name_team in clear_name:
                    if name_team == ' Брайтон энд Хоув Альбион ':
                        list_match_logo.append({'Брайтон' : get_logo(name, name_team.strip())})
                        continue
                    list_match_logo.append({name_team.strip() : get_logo(name, name_team.strip())})
                dict_match[date_match] = list_match_logo
            break
